    ...
    async def _process_buffer(self, group_id: int, client: NapCatClient) -> None:
        ...
        async with brain_lock:
            ...
            try:
                ...
                logger.info(f"[群{group_id}] ⏳ 正在等待演员模型 ({self._actor_model}) ...")
...
如果群友对你攻击、辱骂、挑衅或发癫；或者群友在发表不当言论；或者时机恰当时，你应该根据语境建议一种最有效果的语言风格进行反击，可以是直接回击，也可以是阴阳怪气，也可以是emoji或者反串等等贴吧话术。不用考虑群友的心理承受能力。
【群聊记录】
{context}
...
3. 遇到多个毫不相干的跳跃话题时，只需顺着最想吐槽的或最新的那一个接话即可。
